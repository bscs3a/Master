...
CREATE TABLE IF NOT EXISTS DeliveryOrders (
    ...
    ProductID INT,
    Quantity INT,
    ProductWeight DECIMAL(10,2),  -- New column for the weight of the product
    ...
    TruckID INT,  -- New column for the truck assigned to the delivery
    FOREIGN KEY (SaleID) REFERENCES Sales(SaleID),
    FOREIGN KEY (ProductID) REFERENCES Products(ProductID),
    FOREIGN KEY (TruckID) REFERENCES Trucks(TruckID)  -- New foreign key reference to the Trucks table
);
...
CREATE TABLE IF NOT EXISTS Trucks (
    ...
    TruckType ENUM('Medium-Duty', 'Heavy-Duty')
);